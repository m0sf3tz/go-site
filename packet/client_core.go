...
func Client_handler(conn net.Conn) {
	...
	for {
		...
		case <-cs.err_chan_tcp:
			logger(PRINT_WARN, "ClientID: ", cs.ipc_id, "A TCP error messge was recieved")
		...
		case <-cs.ipc_shutdown:
			logger(PRINT_WARN, "ClientID: ", cs.ipc_id, "shutting down IPC")
			cs.wg.Add(2)
		...
		case tcp_rx := <-cs.tcp_socket_reader_chan:
			logger(PRINT_DEBUG, "ClientID: ", cs.ipc_id, "sending to ipc output")
...
shutdown_client:
	...
	if waitTimeout(&(cs.wg), time.Second) {
		fmt.Println("ClientID: ", cs.ipc_id, "Timed out waiting for wait group")
	} else {
		fmt.Println("ClientID: ", cs.ipc_id, "Wait group finished")
	}

	//TODO: Close IPC and TCP (MUST DO!)
	logger(PRINT_WARN, "ClientID: ", cs.ipc_id, "closing client_handler")
}
...
func client_enqueue_transaction(p Packet, cs *Client_state) {
	...
	logger(PRINT_DEBUG, "ClientID: ", cs.ipc_id, "Transaction_id ", p.Transaction_id, " needs a device ACK, adding it to the TX map")
...
func client_core_handle_packet_rx(p Packet, cs *Client_state) {
	if p.Consumer_ack_req == CONSUMER_ACK_REQUIRED {
		fmt.Println("ClientID: ", cs.ipc_id, "Sending ACK to server for transaction_id", p.Transaction_id)
